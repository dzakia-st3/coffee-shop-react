...
const ButtonBrown = ({ info, onclick }: Readonly<PropsType>) => {
    return (
        <button className="bg-[#6A4029] font-rubik hover:bg-opacity-90 text-white font-semibold px-6 py-1 rounded-sm tracking-wide" onClick={onclick}>{info}</button>
    )
}

const ButtonYellow = ({ info, onclick }: Readonly<PropsType>) => {
    return (
        <button className='bg-[#FFBA33] font-rubik hover:bg-opacity-80 text-[#6A4029] font-semibold px-6 py-2 rounded-sm tracking-wide' onClick={onclick}>{info}</button>
    )
}
...
const GoogleButton = ({ info }: Readonly<PropsType>) => {
    return (
        <div className="shadow-md shadow-gray-500 flex items-center justify-center py-2 px6">
            <img src={Google} alt="" />
            <button className='font-rubik hover:underline hover:decoration-solid text-black font-semibold pl-2 rounded-sm tracking-wide'>{info}</button>
        </div>
    )
}
